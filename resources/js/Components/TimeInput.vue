...
<div class="panel">
    ...
    <input v-model="value" class="form-control bg-white" type="text" :readonly="readonly" :name="name" :class="classes" :id="id" :placeholder="placeholder" @input="save" @change="change"  @blur="save" :required="required" />
     <label> <small v-if="processing" class="fa fa-spinner fa-spin"></small>  {{formartWords(label)}} <span class="text-danger">{{error}}</span> </label>
    </div>
  </div>
</template>
