<template>
 ...
 <Head title="Preferences" />
    ...
    <app-header title="Assets" subtitle="Edit Assets"  />
        <div class="container">
                        ...
                        <div class="tab-pane-content pt-4">
                            <form action="#" class="border  mx-auto p-3">            
                                ...
                                <div class="pt-3  row">  
                                   ...
                                   <div class="col-md-6">                                       
                                       <div class="my-2">
                                           <TextInput :class="{'border-danger':errors.country}" v-on:save="update" v-on:changed="update" label="Asset name" id="asset_name" name="asset_name" type="text"   required  placeholder="Asset name" classes="text-capitalize"  :initVal="asset.name"/>
                                       </div>
                                       <small class="text-danger">{{this.errors.asset_name}}</small>
                                       <div class="my-3">
                                           <TextInput :class="{'border-danger':errors.country}" v-on:save="update" v-on:changed="update" label="Asset type" id="asset_type" name="asset_type" type="text"   required  placeholder="Asset type" classes="text-capitalize"  :initVal="asset.type"/>
                                       </div>
                                      <small class="text-danger">{{this.errors.asset_type}}</small>
                                    </div>
                                   <div class="col-md-6">
                                       <div class="my-2">
                                           <TextInput :class="{'border-danger':errors.country}" v-on:save="update" v-on:changed="update" label="Asset registration no" id="asset_type" name="asset_registration_number" type="text"   required  placeholder="Asset reg no" classes="text-capitalize"  :initVal="asset.asset_registration_no"/>
                                       </div>
                                ...
                                <div class="text-right pb-2 pt-4">
                                    <Link :href="route('employee.assets.form')" class="btn btn-secondary submit px-2 " id="asset-back" > <span class="fa fa-chevron-left"></span> Back</Link>
                                    <button  class="btn btn-theme submit px-5 mx-2" @click.prevent="submitForm()">  Update</button>
                                </div>
                            </form>
...
export default {
    ...
    data(){
        return{
            ...
            asset_registration_number:this.asset_registration_no,   
        }
    },
    methods:{  
        update(response){
        ...
        submitForm(){
            ...
            if(!confirm('Update asset?')) return;

            axios.post('/v1/company/assets/update/'+ this.asset.id, form_data)
            .then(response=>{
